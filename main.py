...
from src.clients.llm_client import generate_meeting_minutes
from src.clients.transcription_client import transcribe_with_groq, transcribe_with_google

TRANSCRIPTION_PATH = ROOT_DIR / "data/transcriptions"
MEETING_MINUTES_PATH = ROOT_DIR / "data/meeting_minutes"
RAW_AUDIO_PATH = ROOT_DIR / "data/raw_audio"
CONFIG_FILE = ROOT_DIR / "config.ini"

logger = get_logger(__name__)

def ensure_directories():
    for path in [TRANSCRIPTION_PATH, MEETING_MINUTES_PATH, RAW_AUDIO_PATH]:
...
def prepare_audio(audio_path: Path) -> Path:
    audio_copy = RAW_AUDIO_PATH / audio_path.name
    audio_copy.write_bytes(audio_path.read_bytes())
    logger.info(f"Copied audio to {audio_copy}")
    return audio_copy
...
def save_transcription(transcript: str) -> Path:
...
def save_meeting_minutes(transcript: str, config, meeting_datetime: datetime) -> Path:
...
def generate_meeting_transcript_and_minutes(audio_path_str: str):
    try:
        ...
        save_meeting_minutes(transcript, config, meeting_datetime)
...
if __name__ == "__main__":
    ...
    generate_meeting_transcript_and_minutes(args.audio_path)
